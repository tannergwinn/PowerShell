...
Foreach ($dg in $DDG)
{
Get-Recipient -RecipientPreviewFilter $DG.RecipientFilter -ResultSize "Unlimited" | Select-Object DisplayName, PrimarySmtpAddress, @{Name=“DDG.Name”;Expression={$dg.Name}} | Export-csv C:\ScriptOutput\$DDG$((Get-Date).ToString('MM-dd-yyyy')).csv
}
